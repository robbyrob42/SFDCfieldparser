...
def get_fields_list_csv(roote):
    with open(sourcefile + '.csv', 'w') as csvfile:
        header_row = ['fullName', 'label', 'externalId', 'type', 'formula', 'formulaTreatBlanksAs', 'unique',
                      'required', 'referenceTo', 'relationshipName', 'relationshipLabel']
        writer = csv.DictWriter(csvfile, fieldnames=header_row, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL,
                                extrasaction='ignore')
        writer.writeheader()
        for e in roote.iter('{http://soap.sforce.com/2006/04/metadata}fields'):
            field_row = {}
            for m in e:
                val = {m.tag[41:]: m.text}
